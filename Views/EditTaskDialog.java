...
public class EditTaskDialog extends JDialog {
    ...
    public EditTaskDialog(JFrame parent, PendingTask task) {
        ...
        // Set initial height
        int initialLineCount = (task.getDescription() != null)
                ? (int) task.getDescription().chars().filter(ch -> ch == '\n').count() + 1
                : 1;
        ...
        pendingDeadlineField.setText(task.getDeadline() != null
                ? task.getDeadline().format(java.time.format.DateTimeFormatter.ofPattern("MM/dd/yyyy"))
                : "");
